...
//Controlling basic PC functions using Hand Gestures


const int trigPin1 = 11; // the number of the trigger output pin ( sensor 1 )
const int echoPin1 = 10; // the number of the echo input pin ( sensor 1(LEFT) )     
const int trigPin2 = 6;  // the number of the trigger output pin ( sensor 2 ) 
const int echoPin2 = 5;  // the number of the echo input pin ( sensor 2  (RIGHT) ) 
...
void loop()
{
  ...
  { 
    ...
    while(distance1<=35 && distance1>=15) // this loop will rotate until we remove our hand infront of the left sensor. this will avoid not to enter this if condition again.
    find_distance();                      
    }
    l=0;                                  // make l=0 for the next round.
   }
   else if((distance1>=50 && distance2 >=50) && (distance1<=60 && distance2<=60))
   {
